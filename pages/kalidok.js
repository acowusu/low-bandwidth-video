import Kalidokit from 'https://cdn.jsdelivr.net/npm/kalidokit@1.1.5/dist/index.min.js'
...
const Kalidok = () => {
  ...
  const randomColors = () => {
    return oneOf(this.colors.slice(3));
  }
  const [colorPrimary, colorSecondary, colorTeriary] = oneOf(palettes)// import * as THREE from "three"
  /* THREEJS WORLD SETUP */
  let currentVrm;

  // renderer
  const renderer = new THREE.WebGLRenderer({ alpha: true });
  renderer.setSize(window.innerWidth / 1.4, window.innerHeight);
  ...
  scene.background = new THREE.Color(colorPrimary);
  // light
